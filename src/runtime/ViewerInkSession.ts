...
export class ViewerInkSession {
  ...
  /** One document-level raw browser contact collector shared by all viewer sessions. */
  ...
  private logPhysicalContactRecords(
    ...
    for (const record of records) {
      ...
      this.logger.pointerSeen({
        ...
        ...this.physicalContactDiagnostics(event),
        phase: record.phase,
  ...
  getUiLifecycleSnapshot(reason = "session-registry"): Record<string, unknown> {
    ...
    const physicalContactCollector = this.physicalContactCollectorLease?.snapshot() ?? null;
