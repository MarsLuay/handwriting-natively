...
describe("PDF page import selection", () => {
  ...
  it("filters the destination PDF and reports picker cancellation", () => {
    const first = { path: "source.pdf", extension: "pdf" };
    const destination = { path: "destination.pdf", extension: "pdf" };
    const cancelled = vi.fn();
    const picker = new PdfImportFilePicker({ vault: { getFiles: () => [first, destination, { path: "note.md", extension: "md" }] } } as never, "destination.pdf", vi.fn(), cancelled);
